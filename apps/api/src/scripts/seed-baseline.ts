 ...
 * with its grants, and one super admin to create the rest from.
 *
 * This is the seed that is safe to run against any environment. It creates
 * no business data and no second account, and the credential it writes is
 * marked `mustChangePassword`, so it reaches login and nothing else until a
 * human rotates it (docs/AUDIT_2026-08-B.md §C2).
 *
 * It lives under `src/` rather than `prisma/` so that `nest build` compiles
 * it into `dist/`, which is what the runtime image ships — a production
 * container has no TypeScript toolchain to run `prisma/seed.ts` with.
 * `prisma/seed.ts` delegates here for local development.
 *
