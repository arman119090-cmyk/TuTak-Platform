...
describe('Money rounding (integration)', () => {
  ...
  describe('EV charging', () => {
    ...
    it('accrues a storable number of points on that bill', async () => {
      ...
      // to a pool of 88.1503. A roaming-CPO session settles like every other purchase
      ...
      expect(after.pendingBonus.plus(after.availableBonus).toString()).toBe('17.63');
    ...
    it('bills a reading whose product needs no rounding, unchanged', async () => {
      ...
      // 5% of 2500 = 125 pool → 25 green (20% of 125, directly — no
      // upfront TuTak cut).
