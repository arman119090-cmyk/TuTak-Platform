...
/** Whole AMD only: nothing a customer types here ever needs fractions. */
...
export function CheckoutScreen() {
  ...
  const discountValue = Math.min(toAmount(discount), maxDiscount);
  const moneyValue = Math.min(toAmount(money), Math.max(total - discountValue, 0));
  const externalValue = Math.max(total - discountValue - moneyValue, 0);
  const missingMoney = Math.max(moneyValue - moneyBalance, 0);
  const prepaymentShort = Math.max(prepayment - discountValue - moneyValue, 0);
