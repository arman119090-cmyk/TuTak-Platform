...
export const partnerOrderApi = {
  ...
  /** Item 8: the customer changes their mind before TuTak decides — the order goes on. */
  async withdrawCancel(id: string) {
    const { data } = await httpClient.post<ApiEnvelope<CustomerPartnerOrderDto>>(`/partner-orders/${id}/cancel/withdraw`);
    return data.data;
  },

  /** Q9: the customer disagrees with a return's shortfall — TuTak reviews; nothing is charged. */
  async disputeReturnShortfall(returnId: string, note: string) {
    const { data } = await httpClient.post<ApiEnvelope<unknown>>(`/partner-orders/returns/${returnId}/dispute-shortfall`, { note });
    return data.data;
  },
