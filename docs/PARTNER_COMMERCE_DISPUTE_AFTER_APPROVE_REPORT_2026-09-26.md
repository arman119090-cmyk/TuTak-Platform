> **Примечание (часть 2, тот же день).** Решениями владельца изменены два пункта
> этого отчёта: автоматический redraft из `revokeApproval` убран (черновики —
> только вручную за закрытый период каденции), revoke из FAILED разрешён при
> доказанном «деньги не ушли», при OPEN споре revoke запрещён. Актуальное
> состояние — `docs/PARTNER_COMMERCE_DISPUTE_AFTER_APPROVE_V2_REPORT_2026-09-26.md`.
> GitHub CI финального head этой части (`814385db`) — 10/10 GREEN.
