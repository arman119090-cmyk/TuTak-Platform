...
**Падение после этого.** На следующем docs-коммите `5a5e27bf` (код тот же)
упал `Integration tests (2/3)` в прогоне push (run 36251550962). Ни один
тест не упал: шаг упёрся в `timeout-minutes: 15`. Выполнено 33 из 39
наборов шарда, все PASS, 6 не успели запуститься. Раннер был в 2–4 раза
медленнее обычного: `purchase-intents` — 154 с против 45 с, `bonus-engine` —
92 с против 20 с на `6e71f4e8`.

Настоящая причина глубже: запаса по времени больше нет. Этот PR добавил
интеграционные наборы Partner Commerce, и шард 1/3 уже в зелёном прогоне
`6e71f4e8` шёл 13,3 минуты из 15. Весь CI `main` на `369eda19` занимал
около 9 минут. Исправление — `.github/workflows/ci.yml`: таймаут шага
`Integration tests` поднят с 15 до 25 минут, с комментарием и этими
цифрами. Это возвращает запас, который комментарий `main` называет целью
(«stall detector, not a performance budget»). Тесты не пропускались и не
отключались. CI на коммите с этим исправлением указан в ответе.
...
- Собственные ошибки по ходу работы:
  ...
  6. Ранее в этой сессии запускал два интеграционных jest одновременно на
     ...
     последовательно.
  7. Объявил CLOSED по зелёному CI на `6e71f4e8`, не проверив запас по
     времени интеграционных шардов. 13,3 минуты из 15 было видно уже в том
     прогоне. Следующий прогон на медленном раннере упал по таймауту, и
     пришлось исправлять CI-конфиг.
