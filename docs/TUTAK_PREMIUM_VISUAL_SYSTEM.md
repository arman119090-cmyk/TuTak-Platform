...
|---|---|---|
| Jako v2 (`V2NavIcon`) | tab bar, referral entry | tab bar 24 px, stroke 2.2 (Pay-диск: 22 px, stroke 2.4); referral entry 26 px, stroke 2.6. Одинаковый для active/inactive; состояние — цветом |
...
Pass 3 (по референсам Revolut / Wise / Airbnb, см.
`PREMIUM_REFERENCES_2026-09-19.md`) применил то же правило ко всем
остальным экранам: Notifications — строки с hairline и точкой
непрочитанного; ScanQr, CreatePurchaseIntent, PurchaseIntentStatus,
EvSession, EvHistory, DeleteAccount — состояния и блоки прямо на
странице; PartnerDetail — одна карточка (обложка), кешбэк/расстояние
как строка под линейкой, адрес/о партнёре/предложения — на странице;
Referral — «пока недоступно» текстом, без белой вставки; чипы категорий,
коннекторов и кнопки степпера — серые `backgroundSubtle` без рамки.
Единственные raised-карточки, что остались: обложка партнёра, Spotlight,
сводка заявки (`subtle`), карточка QR (белая «тихая зона» символа).
...
ведёт на страницу конкретного партнёра (`PartnerDetail` по `partnerId`;
для сети — оттуда «Показать на карте», карта сужена по имени) или, при
`destination = PARTNERS_MAP`, на всю карту — никогда наружу и никогда на
случайный филиал.

**Языки.** Текст карточки (`title`, `subtitle`, `benefitLabel`) хранится
на сервере в `translations` по `hy` / `ru` / `en`; приложение запрашивает
`GET /promos/featured?locale=<язык интерфейса>`. Сервер отдаёт запрошенный
язык → `ru` → первый заполненный; карточка без единого заполненного
языка не отдаётся вовсе (в админке помечена «No language filled»).
Пустой перевод не ломает карточку — он просто не выбирается.

**Загрузка.** Запрос идёт параллельно кошельку и операциям и никогда не
блокирует Home: пока данных нет или запрос упал — секции нет, без
скелетона и без плейсхолдера «нет предложений».
...
`neutral[500]` (400 читался как disabled). Подписи 11/500, line-height 14, letter-spacing −0.2, `textAlign`
center, `minWidth` 88 при `overflow: visible` у item (ширина item — 78 pt
на 390 и 72 pt на 360), `adjustsFontSizeToFit` с `minimumFontScale` 0.9,
одна строка. Правило для армянского «Դրամապանակ» (90 pt при 11 pt):
сначала ширина раскладки, потом раскладка подписи, потом равномерное
уменьшение ≤ 10 %, и только затем — выбор перевода с владельцем; своё
сокращение не выдумывать. При 360 pt соседние подписи «Գլխավոր» и
«Դրամապանակ» расходятся на ~5 pt — проверяется на устройстве. Back в
`Screen` — chevron 26 в круге 36 с pressed-фоном.

## Dates (`format.ts`)

Один форматтер на всё приложение (`formatDate`, `formatDateTime`,
`formatDayGroup`), язык — язык интерфейса (`i18n.language`), не локаль
ОС: «19 сент. 2026 г.» / «19 սեպ, 2026 թ.» / «Sep 19, 2026», время всегда
24-часовое, «Сегодня»/«Вчера» — из словаря. Если рантайм не знает
армянской локали (`Intl.DateTimeFormat.supportedLocalesOf(['hy'])`
пуст — так ведёт себя Chromium из Playwright, который молча отвечает
по-английски), армянская дата собирается вручную из тех же коротких
названий месяцев, что печатает ICU. Экраны: Home, Wallet, история
операций, уведомления, рефералы, статус покупки, EV history.
