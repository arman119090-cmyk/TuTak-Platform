...
* Каждый пункт U01–U12 подтверждён чтением кода на `4c4827e` до правки
  (см. таблицу).
* PR #63 в `claude/dark-theme`. Последний коммит с кодом — `e496f65`; всё,
  что после него, — правки этого отчёта, поэтому голова ветки при чтении
  может быть новее. Код: `0352bc4` (панель партнёра), `46d9a7f` (PSP-контракт
  и экран), `47a6a6d` (история и экран операции), `3d8fbe4` (кошелёк, суммы,
  партнёры), `ca4b47f` (телефон, демо-маршрут), плюс `362b87f` и `e496f65` —
  две починки CI, см. раздел 5. Отчёт и скриншоты: `24037ce` и далее.
